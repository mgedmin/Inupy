...
class Profiler(object):
    def __init__(self, app, config=None, profile_path=None,
                 ignored_paths=DEFAULT_IGNORED_PATHS, **kwargs):
        """Profiles an application and saves the pickled version to a
        file
        """
        # check for the profile path in the config vs the kwargs
        self.inupy_config = config
        if profile_path is None:
            if 'profile_path' in self.inupy_config:
                profile_path = self.inupy_config['profile_path']
    ...
    def render(self, name, **vars):
        ...
        resp = tmpl.render(**vars)

        # inject in the js library code
        return resp.replace('inupy_js', get_js_code(self.tmpl_dir))
